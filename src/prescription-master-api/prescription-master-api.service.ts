...
export class PrescriptionMasterApiService {
  ...
  async create_prescription(CreatePrescriptionMasterApiDto: CreatePrescriptionMasterApiDto[]) {
  ...
  async byProfessionalPrescriptionSummary() {
    ...
    try{
        ...
        if (!promotionInfo) {
          throw new NotFoundException('Prescription List not found.');
        }        
  ...
  async byProfessionalPrescriptionImageList(params) {
    ...
    try{
      ...
      const prescriptionImageList = await this.masterPrescriptionListRepository
        ...
        if (!prescriptionImageList) {
          throw new NotFoundException('Prescription Image List not found.');
        }        
  ...
  async base64_to_image (base64Data: string[], image_path: string, prescription_id: number, created_by: number) {
